...
def extractflat2(npzfile,dx,dy,verbose=False):

    if verbose: print('From extractflat ...')
    ...
    if verbose: print('...shape of solution ', np.shape(solution))
    ...
    for isegment in range(0,nsegments):
        ...
        # Extract this segment
        if verbose: print('...Working on isegment ', isegment)
        nx1=nx1list[isegment]; nx2=nx2list[isegment]; nxsegment = nx2-nx1
        if verbose: print('...nx1,nx2 ', nx1, nx2)
        ny1=ny1list[isegment]; ny2=ny2list[isegment]; nysegment = ny2-ny1
        if verbose: print('...ny1,ny2 ', ny1, ny2)
        if verbose: print('...nxsegment, nysegment ', nxsegment, nysegment)
        surf_xseg = np.linspace(0,(nxsegment-1)*dx,nxsegment); 
        surf_yseg = np.linspace(0,(nysegment-1)*dy,nysegment); 
        surf_xseggrid, surf_yseggrid = np.meshgrid(surf_xseg,surf_yseg) # 1st index is y, 2nd is x
        if verbose: print('...Shape of x and y grids ',np.shape(surf_xseggrid),np.shape(surf_yseggrid))
        ixstart = nx1-ixorigin; ixstop = nx2-ixorigin
        if verbose: print('...ixstart, ixstop ', ixstart,ixstop)
        iystart = ny1-iyorigin; iystop = ny2-iyorigin
        if verbose: print('...iystart, iystop ', iystart,iystop)
        # dummy1 = solution[2][0:nysegment,0:nxsegment]
        dummy1 = solution[2][iystart:iystop,ixstart:ixstop]
        if verbose: print('...Shape of dummy1 ', np.shape(dummy1))
        surf_zseggrid = np.flipud(dummy1) # This flips the y-coordinate
        if verbose: print('...Shape of z grid ',np.shape(np.shape(surf_zseggrid)))
